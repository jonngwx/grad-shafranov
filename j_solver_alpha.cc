/*!
 * @file j_solver_alpha.cc
 * @brief Implementation for JSolverAlpha
 */
#include <assert.h>
#include <stdio.h>
#include <math.h>
...
JSolverAlpha::JSolverAlpha(double P0, double g0, double n1, double n2,
                           double Ip, Grid *grid)
    : n1_(n1), n2_(n2) {
  P0_ = P0;
  g0_ = g0;
...
void JSolverAlpha::update(Field *jphi, Field *psi, Field *p, Field *g) {
  ...
  double alpha_g = mu0 * (-P0_ * temp1 + Ip_ * delta_psi / (dr_ * dz_)) /
                   (0.5 * g0_ * g0_ * temp2);
  // printf("alpha_g = %f \n", alpha_g);
